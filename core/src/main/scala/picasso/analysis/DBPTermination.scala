...
trait DBPTermination[P <: DBCT] {
  ...
  protected def coverOrFold(from: S, morph: Map[P#V, P#V], to: S, comment: String) = {
    ...
    val stmts1 = for ( (node, lst) <- backwardFolding ) yield {
       ...
       getCardinality(map2, node) match {
         case v @ Variable(_) => Eq(v, rhs)
         case other =>
           Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
       }
    }
    //With Transition, the from variables are not in the target so we don't need them.
    ...
    val stmts3 = for (node <- frame) yield {
      getCardinality(map2, node) match {
         case v @ Variable(_) => Eq(v, Constant(0))
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
      }
    }
    val stmts = (stmts1 /*++ stmts2*/ ++ stmts3).toList
    val guard = guardForConcreteNode(from)
    val idMap1 = map1.values.map(x => (x,x)).toMap
    val idMap2 = map2.values.map(x => (x,x)).toMap
    new Transition(pc1, pc2, idMap1, idMap2, And(guard :: stmts), comment)
  }

  // summing up the variable and adding constant for non-replicated nodes.
  protected def folding(from: S, folding: Map[P#V, P#V], to: S) = {
    Logger("DBPTermination", LogDebug, "folding transition from " + from +
                                       " to " + to +
                                       "\nfrom.vertices: " + from.vertices +
                                       "\nto.vertices: " + to.vertices)
    coverOrFold(from, folding, to, "folding")
  }
  
  protected def covering(smaller: S, morph: Map[P#V, P#V], bigger: S) = {
    ...
    coverOrFold(smaller, morph, bigger, "covering")
  }

  // replicated nodes that disappear are set to 0
  // node from 'from' should be either in 'to' or in 'inhibited'
  protected def inhibiting(from: S, inhibited: Set[P#V], flattening: Map[P#V,P#V], to: S) = {
    ...
    val stmts1 = for ( node1 <- from.vertices if !inhibited.contains(node1) ) yield {
      ...
      getCardinality(map2, node2) match {
         case v @ Variable(_) => Eq(v, getCardinality(map1, node1))
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
      }
    }
    ...
    //  }
    //}
    val stmts = (stmts1 /*++ stmts2*/).toList
    val idMap1 = map1.values.map(x => (x,x)).toMap
    val idMap2 = map2.values.map(x => (x,x)).toMap
    new Transition(pc1, pc2, idMap1, idMap2, And(guardForConcreteNode(from) :: stmts), "inhibiting")
  }

  //TODO when unfolding a component, do we need to keep some more information ? (i.e. unfold the same as or at least as much as ...)
  protected def unfolding(from: S, backwardUnFolding: Map[P#V, Seq[P#V]], to: S) = {
    ...
    */
    ...
    val variance1 = increases.map( n => {
      getCardinality(map2, n) match {
         case v1 @ Variable(_) => Leq(Constant(0), v1)
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
      }
    })
      ...
      getCardinality(map2, nNew) match {
         case v1 @ Variable(_) =>
           getCardinality(map1, nOld) match {
             case v2 @ Variable(_) => List( Leq(Constant(0), v1), Leq(v1, v2) )
             case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
           }
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
      }
    ...
    val guard = guardForConcreteNode(from) :: lowerBounds.toList
    val stmts = (stmts1.flatten /*++ stmts2*/ ++ variance1 ++ variance2).toList
    val idMap1 = map1.values.map(x => (x,x)).toMap
    val idMap2 = map2.values.map(x => (x,x)).toMap
    new Transition(pc1, pc2, idMap1, idMap2, And( guard ::: stmts), "unfolding")
  }

  //Just check that the transition does not change replicated things and transfer the frame
  protected def morphing(from: S, frame: Map[P#V, P#V], tr: T, to: S) = {
    ...
    */
    val stmts2 = for (n <- appearing) yield {
       getCardinality(map2, n) match {
         case v @ Variable(_) => if (n.depth == 0) Eq(v, Constant(1)) else Leq(Constant(0), v)
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
       }
    }
    val stmts3 = for ((n1,n2) <- frame) yield {
       getCardinality(map2, n2) match {
         case v @ Variable(_) => Eq(v, getCardinality(map1, n1))
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
       }
    }
    /*
    ...
    */
    val stmts = (/*stmts1 ++*/ stmts2 ++ stmts3 /*++ stmts4*/).toList
    val idMap1 = map1.values.map(x => (x,x)).toMap
    val idMap2 = map2.values.map(x => (x,x)).toMap
    new Transition(pc1, pc2, idMap1, idMap2, And(guardForConcreteNode(from) :: stmts), "morphing, " + tr.id)
  }

  protected val emptyConf = DepthBoundedConf.empty[P]

  protected def initialize(init: S) = {
    ...
    val stmts = for (node <- init.vertices) yield {
       getCardinality(map1, node) match {
         case v @ Variable(_) =>
           if (node.depth == 0) Eq(v, Constant(1))
           else Leq(Constant(0),v)
         case other => Logger.logAndThrow("DBPTermination", LogError, "Expected Variable, found: " + other)
       }
    }
    val idMap1 = map1.values.map(x => (x,x)).toMap
    new Transition(pc0, pc1, Map.empty, idMap1, And(stmts.toList), "initialize")
  }

  protected def transitionForWitness(witness: TransitionWitness[P]) = {
    val t1 =
  ...
  protected def simplifyPathByQE(trs: Seq[Transition]) = {
    Transition.compact(trs)
  }
