...
object Copier {
  ...
  def Literal[T](from: Formula, value: T) = {
    val l2 = picasso.math.hol.Literal(value)
    l2.tpe = from.tpe
    l2
  }

  def Variable(from: Formula, name: String) = {
    val v2 = picasso.math.hol.Variable(name)
    v2.tpe = from.tpe
    v2
  }

  def Application(from: Formula, fct: Formula, args: List[Formula]) = {
    val a2 = picasso.math.hol.Application(fct, args)
    a2.tpe = from.tpe
    a2
  }

  def Binding(from: Formula, bt: BindingType, vs: List[Variable], f: Formula) = {
    val b2 = picasso.math.hol.Binding(bt, vs, f)
    b2.tpe = from.tpe
    b2
  }

  def Lambda(from: Formula, vs: List[Variable], f: Formula) = {
    val l2 = picasso.math.hol.Lambda(vs, f)
    l2.tpe = from.tpe
    l2
  }

  def Exists(from: Formula, vs: List[Variable], f: Formula) = {
    val e2 = picasso.math.hol.Exists(vs, f)
    e2.tpe = from.tpe
    e2
  }

  def ForAll(from: Formula, vs: List[Variable], f: Formula) = {
    val a2 = picasso.math.hol.ForAll(vs, f)
    a2.tpe = from.tpe
    a2
  }
...
abstract class Transformer {
  ...
  def transform(f: Formula): Formula = f match {
    ...
    case f @ Application(fct, args) =>
      ...
      Copier.Application(f, fct2, args2)
